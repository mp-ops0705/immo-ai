...
// Compute remaining loan balance from static loan params + start date + today.
// Uses iterative amortization so there is no floating-point drift vs the analytical formula.
...
const initial: FormState = {
  ...
  notary_fees: '', works_cost: '',
  has_loan: false, initial_loan_amount: '', remaining_loan: '', monthly_payment: '', loan_rate: '', loan_duration_years: '', loan_start_date: '',
  is_rented: false, rent: '', charges: '',
  taxe_fonciere: '', charges_copro: '', frais_gestion_pct: '', assurance_emprunteur: '', autres_charges: '',
};
...
function fmtDate(v: string | null) {
  if (!v) return '—';
  try { return new Date(v).toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' }); } catch { return v; }
}
...
export default function AjouterBienPage() {
  ...
  const applyLoanImport = () => {
    ...
    // Calculate remaining_loan mathematically instead of trusting the AI-extracted value
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      purchase_date: parseDate(form.purchase_date),
      ...
      loan_duration_years: form.has_loan ? (parseNum(form.loan_duration_years) !== null ? Math.round(parseNum(form.loan_duration_years)!) : null) : null,
      loan_start_date: form.has_loan ? parseDate(form.loan_start_date) : null,
    ...
    setIsSaving(false);
    if (dbError) { setError('Erreur lors de l\'enregistrement. Réessaie.'); return; }
    router.replace('/dashboard');
  };

  const confidenceColor = importResult
    ? importResult.confidence_score >= 0.8 ? '#16a34a' : importResult.confidence_score >= 0.6 ? '#d97706' : '#e11d48'
    : '#64748b';
    ...
    <main style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #e8edf5 0%, #f8fafc 260px, #f8fafc 100%)', padding: '12px' }}>
      <section style={{ width: '100%', maxWidth: '430px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '16px', paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 30px)' }}>
        ...
        <header style={{ padding: '20px 24px 20px', background: 'linear-gradient(145deg, #0f172a 0%, #1e293b 100%)', color: '#ffffff', position: 'relative', overflow: 'hidden', marginLeft: '-12px', marginRight: '-12px', marginTop: '-12px', display: 'flex', alignItems: 'center', gap: '14px' }}>
          ...
          <button type="button" onClick={() => router.back()} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '10px', padding: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, zIndex: 1 }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth={2.2} strokeLinecap="round" strokeLinejoin="round"><path d="M15 18l-6-6 6-6" /></svg>
          </button>
          <div style={{ zIndex: 1 }}>
            ...
            <h1 style={{ margin: 0, fontSize: '22px', fontWeight: 900, letterSpacing: '-0.02em' }}>Ajouter un bien</h1>
          </div>
        </header>
        ...
        {/* ── Crédit ── */}
        <div style={sectionStyle}>
          {/* Toggle header — clean, no import button here */}
              ...
              {importStatus === 'success' && importResult && (() => {
                ...
                <div style={{ padding: '14px', borderRadius: '12px', backgroundColor: '#f8faff', border: '1.5px solid #c7d2fe', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    ...
                    {/* Always ask for start date manually */}
        ...
        {/* ── Revenus locatifs ── */}
        <div style={sectionStyle}>
          <button type="button" onClick={toggle('is_rented')} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'none', border: 'none', padding: 0, cursor: 'pointer', width: '100%' }}>
            <div>
        ...
        {/* ── Save ── */}
        <button type="button" onClick={handleSave} disabled={isSaving} style={{ padding: '16px', borderRadius: '14px', background: isSaving ? '#94a3b8' : 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', color: '#ffffff', fontSize: '15px', fontWeight: 800, border: 'none', cursor: isSaving ? 'not-allowed' : 'pointer', boxShadow: '0 4px 16px rgba(15,23,42,0.2)', letterSpacing: '-0.01em' }}>
          {isSaving ? 'Enregistrement…' : 'Enregistrer le bien'}
        </button>
